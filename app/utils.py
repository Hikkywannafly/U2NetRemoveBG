...
# Kích thước ảnh thẻ tiêu chuẩn (mm)
PHOTO_SIZES = [
...
def mm_to_pixels(mm, dpi=DPI):
    """Chuyển đổi từ mm sang pixel dựa trên DPI"""
    return int((mm / 25.4) * dpi)

def generate_unique_filename(original_filename):
    """Tạo tên file duy nhất bằng cách thêm UUID vào tên file gốc"""
    # Lấy phần mở rộng của file
    _, ext = os.path.splitext(original_filename)
    # Tạo UUID ngẫu nhiên
    unique_id = str(uuid.uuid4())
    # Kết hợp UUID với phần mở rộng
...
def create_id_photo(input_path, output_path, size_name="3x4", bg_color=(255, 255, 255)):
